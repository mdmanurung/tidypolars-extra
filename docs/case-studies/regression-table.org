...
And here is how to run the estimation in tidypolars-extra and produce a table with tidy results (click on the (+) sign to see code comments):
...
We can group the rows in the table by post-processing the ~tibble~ outcome from the ~models2tab()~ function using tidypolars-extra function ~to_latex()~. Something like this:
...
We need to post-process the ~tibble~ outcome from the ~models2tab()~ function using tidypolars-extra function ~to_latex()~. The code:
